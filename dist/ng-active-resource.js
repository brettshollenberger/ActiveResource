...
Function.prototype.extend = function (Module, options) {
  var properties = new Module(), options = options || {}, propertyNames = Object.getOwnPropertyNames(properties), classPropNames = _.remove(propertyNames, function (propName) {
...
angular.module('ngActiveResource').factory('ARAPI', [
  ...
  function (querystring, Mime, serializeAssociations) {
    ...
    API.prototype.perPageAttribute = 'per_page';
    API.prototype.wrapRoot = false;
    API.prototype.unwrapRoot = false;
    ...
    function API(klass) {
      ...
      api.generateRequest = function (action, config) {
        var config = config || {}, params = config.params || {}, urlAndParams = parameterize(getEndpoint(action), params, isGETURL(action)), url = urlAndParams[0], params = urlAndParams[1];
      ...
      function parameterize(url, params, isGETURL) {
        ...
        unparameterized = unparameterizable(url, params);
      ...
      function unparameterizable(url, params) {
        return _.chain(params).cloneDeep().omit(_.isUndefined).omit(parameterizable).value();
...
angular.module('ngActiveResource').factory('ARDefinable', [
  'AREventable',
  function (Eventable) {
...
angular.module('ngActiveResource').factory('ARDirty', [
  'ARMixin',
  'ARSerializeAssociations',
  function (mixin, serializeAssociations) {
    ...
    function attributeChanged(attributeName, currentAttributes, lastSave) {
      if (_.isUndefined(currentAttributes[attributeName])) {
        return false;
      }
...
angular.module('ngActiveResource').factory('AREventable', [
  'ARMixin',
  'ARFunctional.Collection',
  function (mixin, FunctionalCollection) {
    function Eventable() {
      ...
      function EventRunner(context, eventType, handler, handlerArgs) {
        privateVariable(context, 'unbind', function () {
          events.handlers[eventType].$reject(function (h) {
            return h == handler;
          });
        });
...
angular.module('ngActiveResource').factory('ARFunctional.Collection', [
  ...
  function (mixin) {
    function FunctionalCollection() {
      ...
      privateVariable(this, 'clone', function () {
...
angular.module('ngActiveResource').factory('ARHTTPResponseHandler', [
  ...
  function (strictRequire) {
    ...
    function emit(message, options) {
      options.klass.emit(options.action + ':' + message, options.deferred, options.response, options.request);
      options.deferred.emit(options.action + ':' + message, options.deferred, options.response, options.request);
    }
...
angular.module('ngActiveResource').factory('ARCreatable', [
  'AREventable',
  function (Eventable) {
    function Creatable() {
  ...
  function ($http, HTTPResponseHandler, httpConfig) {
    function Deletable() {
      this.__$delete = function (config) {
        var instance = this, klass = instance.constructor, config = _.merge({}, config, httpConfig(klass), _.defaults), url = klass.api.generateRequest('delete', { params: _.cloneDeep(instance) });
  ...
  function ($http, HTTPResponseHandler, httpConfig) {
    function Findable() {
      this.find = function (terms, config) {
        var klass = this;
        terms = standardizeTerms(), config = _.merge({ params: terms }, config, httpConfig(klass), _.defaults);
        ...
        function returnRemote() {
          var instance = klass.new(), url = klass.api.generateRequest('show', config);
  ...
  function ($http, mixin, FunctionalCollection, Paginatable, Promiseable, HTTPResponseHandler, httpConfig, Refinable) {
    function Queryable() {
      ...
      this.where = function (terms, config) {
        var klass = this, queryResults = new Refinable(klass);
...
//
angular.module('ngActiveResource').factory('ARPaginatable', [
  'ARMixin',
  'ARFunctional.Collection',
  function (mixin, FunctionalCollection) {
    function Paginatable() {
      ...
      this.page_exists = function (nextOrPrevious) {
        var pageNumber = _.isObject(paginationHypermedia[nextOrPrevious]) ? paginationHypermedia[nextOrPrevious].params[this.paginationAttribute()] : undefined;
      ...
      this.parseHypermedia = function () {
        if (_.isUndefined(this.__headers)) {
          return;
        }
      ...
      this.parseHypermedium = function (hypermedia, nextOrPrevious) {
        if (!_.isObject(hypermedia)) {
          return;
        }
        var pa = this.paginationAttribute();
        if (_.isUndefined(hypermedia[nextOrPrevious]) || _.isUndefined(paginationHypermedia[nextOrPrevious]) || compare()) {
          paginationHypermedia[nextOrPrevious] = hypermedia[nextOrPrevious];
        }
        function compare() {
          return nextOrPrevious == 'next' ? hypermedia.next.params[pa] > paginationHypermedia.next.params[pa] : hypermedia.previous.params[pa] < paginationHypermedia.previous.params[pa];
        }
      };
    ...
    function toHypermedia(LinkHeader) {
      ...
      return _.inject(_.zipObject(mixin(LinkHeader.split(','), FunctionalCollection).map(function (rel) {
    ...
    function url2obj(url) {
      url = url.trim();
      var baseURL = url.split('?')[0], paramString = url.split('?')[1], params = {};
      if (!_.isUndefined(paramString)) {
        params = _.inject(paramString.split('&'), function (params, param) {
          var keyVal = param.split('='), key = keyVal[0], val = keyVal[1];
          if (!isNaN(val)) {
            val = Number(val);
          }
          params[key] = val;
...
angular.module('ngActiveResource').factory('ARQueryCache', [
  'ARMixin',
  'ARFunctional.Collection',
  function (mixin, FunctionalCollection) {
    function QueryCache() {
      ...
      privateVariable(this, 'where', function (query) {
        return _.chain(this).keys().map(JSON.parse).select(function (cachedQuery) {
          return _.all(query, function (val, key) {
            return cachedQuery[key] == val;
          });
        }).value();
      });
...
angular.module('ngActiveResource').factory('ARRefinable', [
  '$http',
  'ARMixin',
  'ARFunctional.Collection',
  'ARHTTPConfig',
  ...
  function ($http, mixin, FunctionalCollection, httpConfig, Promiseable, HTTPResponseHandler, QueryCache, Paginatable, Eventable) {
    function Refinable(klass) {
      ...
      function createOrUpdate(instances) {
        return mixin(_.map(instances, function (instance) {
          return klass.createOrUpdate(instance);
        }), FunctionalCollection);
      }
      function refine(instances) {
        refinable.$removeAll();
        _.each(instances, function (instance) {
          refinable.push(instance);
        });
        return refinable;
      }
      function onSuccess(success) {
        var newInstances = always(success);
        if (!success.config.preload) {
          refine(newInstances);
        }
      }
      ...
      function nonPaginationParamsChanged(params) {
        return _.chain(params).keys().reject(function (param) {
          return param == refinable.perPageAttribute() || param == refinable.paginationAttribute();
        }).any(function (param) {
          if (!_.isObject(refinable.mostRecentCall)) {
            return false;
          }
  ...
  function ($http, httpConfig, HTTPResponseHandler) {
    function Saveable() {
      this.__$save = function (attributes, config) {
        ...
        instance.emit('save:called', attributes);
        instance.defer();
        instance.update(attributes);
        if (instance.$valid) {
          var url = klass.api.generateRequest(createOrUpdate(), { params: _.cloneDeep(instance) });
...
angular.module('ngActiveResource').factory('ARSerializeAssociations', [
  ...
  function (foreignkeyify, dropHasMany) {
    return function serializeAssociations(instance, reflections) {
      var reflections = (reflections || instance.constructor.reflections).clone(), clone = _.cloneDeep(instance);
  ...
  function (serializeAssociations) {
    ...
    function Serializable() {
      ...
      function wrapRoot(object, klass) {
        var wrapperName;
        if (_.isArray(object)) {
          wrapperName = klass.api.resourcesName();
        } else {
          wrapperName = klass.api.resourceName();
        }
        wrapped = {};
