...
//    }
//
// NOTICE: In the example above, we say that the player's "lost" property relies on the ship's "sunk" property, not its
// "state" property.
//
// This is because "sunk" is itself a computed property. If Player listened for ship#state and Ship listened for ship#state, then
// we would run the risk of having the Player callback fire before the Ship callback.
//
// Since Player _actually_ relies on knowing whether or not the Ship has sunk, and not on whether or not its state has changed in
// general, we must ensure that the ship#sunk dependency has finished evaluating before the player#lost callback fires.
//
