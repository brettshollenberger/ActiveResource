...
angular
  ...
  .factory('ARAPI', ['ARQueryString', 'ARMime', 'ARSerializeAssociations',
    ...
    function API(klass) {
      ...
      api.generateRequest = function(action, config) {
        var config        = config        || {},
            ...
            urlAndParams  = parameterize(getEndpoint(action), params, isGETURL(action)),
            url           = urlAndParams[0],
            params        = urlAndParams[1];

        config.params = params;
        if (_.isEmpty(config.params)) { delete config.params; }
      ...
      function getEndpoint(action) {
        ...
        return standardizeBaseURL(api.baseURL) +
               api.nestedURLComponent() +
               standardizeEndpoint(endpoint);
      }

      function parameterize(url, params, isGETURL) {
        if (klass.reflections) { params = serializeAssociations(params, klass.reflections); }
      ...
      function prependSlash(endpoint) {
        if (endpoint.slice(0, 1) != "/") { return "/" + endpoint; }
        return endpoint;
      }

      function appendSlash(endpoint) {
        if (endpoint.slice(-1) != "/") { return endpoint + "/"; }
        return endpoint;
      }
    }
