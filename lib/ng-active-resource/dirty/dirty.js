  ...
  .factory('ARDirty', ['ARMixin', 'ARSerializeAssociations', function(mixin, serializeAssociations) {
    ...
    function Dirty() {
      ...
      privateVariable(this, 'buildDirtyAttributes', function() {
        var propertyNames     = _.keys(this),
            currentAttributes = _.cloneDeep(this),
            klass             = this.constructor,
            reflections       = klass.reflections || {},
            snakeOrCamel      = snakeOrCamelcase(propertyNames);

        _.each(propertyNames, function(propertyName) {
          var changedName      = caseify(propertyName, "changed", snakeOrCamel),
              wasName          = caseify(propertyName, "was", snakeOrCamel);
    ...
    function snakeOrCamelcase(propertyNames) {
      if (_.all(propertyNames, function(propertyName) { return isSnake(propertyName); })) {
        return "snake";
      }

      if (!_.any(propertyNames, function(propertyName) { return containsUnderscore(propertyName); })) {
        return "camel";
      }
    }
    ...
    function caseify(propertyName, toAppend, snakeOrCamel) {
      ...
      return thisSnakeOrCamel == "snake" ? propertyName + "_" + toAppend.toLowerCase() :
                                           propertyName + toAppend.capitalize();
    }
