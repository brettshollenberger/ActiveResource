xdescribe("ARPaginatable", function() {
  ...
  describe("Starting in the middle of a list", function() {
    ...
    iit("resets previously loaded pagination information when pagination params change back and forth", function() {
      posts.where({author_id: 2, per_page: 1});
      backend.flush();
      expect(posts.first().id).toEqual(200);
      ...
      $timeout.flush();
      expect(posts.pluck("id")).toEqual([6, 7, 8, 9, 10]);
      expect(posts.paginationHypermedia().next.params).toEqual({author_id: 1, page: 3, per_page: 5});
      // expect(posts.paginationHypermedia().previous.params).toEqual({author_id: 1, page: 1, per_page: 5});
    });
  });
});
